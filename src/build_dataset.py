# src/build_raw_dataset_circle_alt.py
...
# --- PERUBAHAN: Dari persegi ke lingkaran ---
AREA_RADIUS_KM = 500 # Jari-jari untuk area UT
N_SAMPLES = 10000
N_UT = 10
...
# --- FUNGSI BARU: Untuk menghasilkan posisi dalam lingkaran ---
def generate_random_circular_positions(center_lat, center_lon, radius_km, num_points):
    """
    Menghasilkan koordinat (lat, lon) secara acak di dalam area lingkaran.
    """
    lat_degree_per_km = 1.0 / 111.0
    lon_degree_per_km = 1.0 / (111.0 * np.cos(np.radians(center_lat)))

    # Menghasilkan radius dan sudut acak untuk distribusi yang seragam
...
def generate_random_square_positions(center_lat, center_lon, side_km, num_points):
    """
    Menghasilkan koordinat (lat, lon) secara acak di dalam area persegi (untuk Satelit).
    """
    lat_degree_per_km = 1.0 / 111.0
    lon_degree_per_km = 1.0 / (111.0 * np.cos(np.radians(center_lat)))
...
def main():
    ...
    for n_sats in N_SATS_LIST:
        # --- PERUBAHAN: Nama file output disesuaikan ---
        save_path = os.path.join(SAVE_DIR, f"raw_data_{n_sats}satelite_circle.csv")
        ...
        with open(save_path, "w", newline="") as f:
            ...
            for sample_id in tqdm(range(N_SAMPLES), desc=f"Skenario {n_sats} Satelit"):
                # --- PERUBAHAN UTAMA: Menggunakan fungsi lingkaran untuk UT ---
                ut_positions = generate_random_circular_positions(
                    REGION_CENTER_LAT, REGION_CENTER_LON, AREA_RADIUS_KM, N_UT
                )
